...
3. Reachable from the local machine (via dnsmasq) and from the tailnet (via a Tailscale sidecar with split DNS), without the local machine joining the tailnet.
...
- Joins networks: `dev-ingress` (reach devcontainers), `ts-ingress` (receive from tailscale). Publishes 443 (and 80 for redirects) on the host for local access.

### 5.2 `t3code-relay` Caddy module

Caddy module namespace: `t3code-relay`. It plays two roles:

**a) Dynamic route source (Caddy app + config generation).**
...
- Emits Caddy routes: `https://<container-name>.t3.<domain>` → `reverse_proxy <container-ip>:3773`. Hostname derives from the container `--name` (sanitized); override via an optional label `t3relay.host=<name>`.
- Route emission must compose with caddy-docker-proxy's generated config rather than replace it. Decide mechanism during implementation: either (preferred) implement as a caddy-docker-proxy *label generator sibling* — i.e., the module produces config the same way CDP does and both merge — or run as an independent Caddy app that pushes route updates via Caddy's admin/config API into a dedicated server block. Spike this first (open item D3).
...
- Networks: `dev-ingress` (external — created once by the operator, shared with devcontainers), `ts-ingress` (internal to the compose project).
- `tailscale` service: official `tailscale/tailscale` image; `TS_AUTHKEY` from env; joins `ts-ingress` only; forwards TCP 443 to `caddy:443` (via `tailscale serve`-style config or socat — implementation's choice, but configured declaratively). The local machine is **not** on the tailnet; tailnet peers reach devcontainers only through this node.
- **Tailscale configuration is provided via the Compose top-level `configs:` section with inline `content:`** (no bind mounts), mounted into the container. This includes the serve/forward config.
- `caddy` service: the GHCR image from §5.1; `ports: 443:443, 80:80` on the host; volumes for docker.sock (ro), caddy data, relay sqlite, plus a read-only bind mount of `${RELAY_SECRET_FILE}` → `/run/secrets/t3relay-secret`; env from `.env`.
- Volumes: `caddy-data`, `relay-db`, `tailscale-state`.
...
# Tailscale auth key for the sidecar node
TS_AUTHKEY=
```
...
- Tailscale admin: split DNS for `t3.<domain>` → the sidecar node's tailnet IP.
...
5. Tailscale sidecar + split DNS docs; polish, README, docs/ per §9, tag v0.1.0.
