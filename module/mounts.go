...
.breadcrumb{display:flex;align-items:center;gap:6px;min-width:0;color:var(--muted)}.crumb{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.seg{display:flex;border:1px solid var(--line);height:32px}.seg button{border:0;background:var(--panel);color:var(--muted);padding:0 12px;cursor:pointer}.seg button.active{background:var(--accent);color:white}.token{width:min(420px,34vw);height:32px;border:1px solid var(--line);background:var(--panel);color:var(--ink);padding:0 10px}
...
<div id="shell" class="shell">
  ...
  <main class="main">
    <div class="topbar"><button class="icon" id="expand" title="Show sidebar">&#9776;</button><div id="breadcrumb" class="breadcrumb"><span class="crumb">No file selected</span></div><div class="grow"></div><input id="token" class="token" type="password" autocomplete="off" placeholder="Bearer token"><div class="seg"><button id="renderMode" class="active">Render</button><button id="sourceMode">Source</button></div></div>
...
async function api(path){const r=await fetch(path,{headers:authHeaders()});if(r.status===401)throw new Error("Enter a valid relay bearer token.");if(!r.ok){let msg=r.statusText;try{const e=await r.json();msg=e.reason||e.code||msg}catch{}throw new Error(msg)}return r.json()}
async function loadTree(){try{state.tree=(await api("/v1/mounts/tree")).root;renderTree();}catch(e){el("tree").innerHTML='<div class="error">'+escapeHtml(e.message)+'</div>'}}
...
el("token").onchange=()=>{state.token=el("token").value.trim();localStorage.setItem("t3relay.mounts.token",state.token);loadTree();if(state.path)loadFile()};el("refresh").onclick=loadTree;el("collapse").onclick=()=>el("shell").classList.add("collapsed");el("expand").onclick=()=>el("shell").classList.remove("collapsed");
